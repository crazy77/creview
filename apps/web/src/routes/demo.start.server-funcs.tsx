...
const deleteTodoFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		let todos: Todo[] = await readTodos();
...
function Home() {
	...
	const deleteTodo = useCallback(
		async (id: number) => {
			todos = await deleteTodoFn({ data: id });
			router.invalidate();
		},
		[router, todos],
	);
			...
			<div className="w-full max-w-2xl p-8 rounded-xl backdrop-blur-md bg-black/50 shadow-xl border-8 border-black/10">
				...
				<ul className="mb-4 space-y-2">
					{todos?.map((t) => (
						<li
							key={t.id}
							className="flex justify-between items-center bg-white/10 border border-white/20 rounded-lg p-3 backdrop-blur-sm shadow-md"
						>
